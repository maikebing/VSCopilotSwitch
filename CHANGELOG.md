...
- ✅️ 优化高级选项端口检测：支持 `11434`、`127.0.0.1:11434` 和完整 URL，并将端口格式错误显示在检测结果区，避免误触发全局端口冲突修复建议。
...
- ✅️ VS Code 配置预览和写入改为使用当前运行中的本地代理地址与当前可用模型，不再固定写入 `http://127.0.0.1:11434` 和 `vscopilotswitch/default`。
- ✅️ VS Code/Copilot 暴露模型名新增 `@vscc` 后缀，例如 `gpt-5.5@vscc`，避免与原生模型名冲突；本地代理转发上游前会去掉该后缀。
