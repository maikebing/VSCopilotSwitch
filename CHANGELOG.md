...
- ✅️ 修复 VS2026 HTTPS 访问节点启用后宿主无法启动的问题：CreateSlimBuilder 场景会显式启用 Kestrel HTTPS 配置服务，确保 `https://127.0.0.1:5443` 能和 HTTP 监听同时绑定。
...
- ✅️ 新增端到端人工验收清单，覆盖新增供应商、启用、测试连接、刷新模型、VS Code 写入、Copilot 调用、本地撤销和备份回滚；本轮已执行完整构建、Core/Services/VsCodeConfig 三组测试和命名检查，阶段 5.5 真实功能闭环收尾完成。
...
- ✅️ 分析统计新增 usage / 费用精算：优先解析上游真实 `usage`，支持 OpenAI JSON、OpenAI SSE 和 Ollama 响应形态；费用按本地 `UsagePricing` 单价表以每百万输入/输出 Token 计算，并在 UI 中区分实际 usage、估算 usage、已计价和未计价请求。
