...
- 通过 ASP.NET Core 本地宿主提供管理页和代理 API，当前优先支持 Windows 配置写入；macOS、Linux、WSL 暂不自动写入，后续跨平台阶段再补齐。
...
项目当前采用 ASP.NET Core 本地 Web 宿主和 Vue 3 SPA。开发阶段通过项目内 npm 脚本启动 SPA 调试服务；发布阶段先构建 SPA 静态产物，再作为嵌入式资源打包进 AOT 单体应用，由宿主在运行时加载内置 SPA 资源。实现上建议分层：

- `host`：ASP.NET Core 本地宿主、HTTP API、后台生命周期和 SPA 嵌入式资源加载。
...
当前通过浏览器访问本地管理页完成日常切换：

- 访问 `http://127.0.0.1:5124/` 打开管理界面。
- 首页可查看当前提供商、当前模型和代理状态。
- 供应商列表支持快速启用已保存密钥和模型名的真实提供商。
- 停止宿主进程会停止本地代理。
...
高级选项中的本地代理地址支持端口占用检测，可填写 `5124`、`127.0.0.1:5124` 或完整 URL，并提示 `127.0.0.1` 上的目标端口是否已被其他代理占用。VSCopilotSwitch 不再把 VS Code Provider URL 指向 Ollama 默认的 `11434`，避免与用户本机原生 Ollama 服务冲突。发布包运行时从单体程序内嵌的 SPA 静态资源加载界面，不依赖外部 `wwwroot` 目录。
...
阶段 5 首批 Provider Adapter 首版已完成，阶段 5.5 已把 UI、受保护供应商配置、Provider Adapter 和 Ollama 代理串成真实闭环：当前已支持 UI 启用供应商驱动 `/api/tags` 与 `/api/chat`，并在首页展示真实模型刷新状态、上游模型名和失败原因；供应商测试连接、协议类型选择、VS Code 写入当前代理模型和端到端验收清单也已接入。
...
宿主项目已回归纯 ASP.NET Core 本地 Web 服务，不再引用外部桌面宿主源码。运行时会启动 API / SPA 服务；开发模式优先使用 `launchSettings.json` / `ASPNETCORE_URLS` 中的固定地址，方便 Vite 代理对齐，未显式配置时默认使用 `http://127.0.0.1:5124`。

当前阶段保留本地 HTTP 服务边界，便于 VS Code 配置 API、Ollama 兼容代理和 Vue SPA 继续复用现有开发链路；系统托盘、原生窗口和跨平台桌面壳不再作为当前项目默认能力。
