...
- UI 参考 `cc switch` 的快速切换体验，并采用 VS Code Workbench 风格主题变量，强调模型供应商、模型、密钥、代理和 VS Code 配置的一站式管理。
...
- `ui`：Vue 3 SPA，负责供应商配置、模型切换、状态面板、日志和向导；主题层使用 `--vscode-*` 变量，后续可由 OmniHost 注入或覆盖。
