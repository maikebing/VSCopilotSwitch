...
当前主线：🔴 现在做 阶段 5.5 真实功能闭环。已把“界面保存并启用的供应商”接入 Ollama 代理路由，并让模型列表从当前启用供应商实时刷新；下一步优先新增供应商“测试连接”，验证 Base URL、API Key、模型列表和最小聊天探测。
...
- 上游错误不会直接泄露敏感字段。
- 当前阶段的基础模型切换链路已接入 UI 启用供应商；VS Code 写入当前可用模型、测试连接和托盘快速切换仍在阶段 5.5 后续任务中补齐。
...
- ✅️ 让模型列表从当前启用供应商实时获取，并在 UI 展示真实模型、刷新状态和失败原因。
- 🔴 现在做 新增供应商“测试连接”：验证 Base URL、API Key、模型列表和最小聊天探测，错误必须脱敏。
- ⬜ 保存供应商时支持选择 Provider 类型：OpenAI-compatible、OpenAI Official、DeepSeek、Claude、NVIDIA NIM、MoArk、sub2api。
- ⬜ VS Code 配置写入应使用当前本地代理地址和当前可用模型，而不是固定默认模型。
