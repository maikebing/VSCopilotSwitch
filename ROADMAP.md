...
阶段状态：✅️ 已完成差距分析文档、MewUI 首页控制台首版、阶段 6 健康状态解释入口、供应商预设与导入、模型测试比较、VS Code 配置写入加固和托盘快速切换。下一步进入阶段 6 稳定性与路由主线。

- ✅️ 新增 `docs/cc-switch-gap-analysis.md`，整理当前实现、cc switch 参照点、核心差距、优先级和明确不照搬的多 CLI / MCP / Skills 范围。
- ✅️ 重做 MewUI 首页为“当前生效链路控制台”：当前供应商、当前模型、公开模型名、代理健康、VS Code 配置状态、最近请求结果和关键操作在第一屏可见。
- ✅️ 将供应商启用动作升级为可解释的路由切换：切换后自动刷新模型、健康检查、VS Code 状态，并提示用户 Copilot 侧是否需要重新发现模型。
- ✅️ 衔接阶段 6 的健康状态解释入口：概览页解释 `/health`、供应商可路由性、`/api/tags` 模型列表、最近请求失败原因和 Copilot 健康探针结果；完整重试、熔断和备用路由仍留在阶段 6 主线。
- ✅️ 增加供应商预设和导入能力：常见 OpenAI-compatible、中转站、官方 Provider 的 Base URL、协议类型、模型推荐和能力声明模板；导入预览不自动保存密钥。
- ✅️ 增加模型测试比较：延迟、首 token、流式结束、工具调用探针、上下文声明和费用估算结果可在本次运行内保存并用于选择模型。
- ✅️ 加固 VS Code 配置写入：真正原子写入、JSON with comments 兼容策略、备份保留策略和 Windows/macOS/Linux/WSL 路径测试。
- ✅️ 补齐托盘快速切换：托盘显示当前供应商/模型，支持打开或聚焦窗口、快速切换供应商和退出。
...
- ✅️ 熔断器：失败阈值、半开探测、自动恢复。
- 🔵 后做 重试策略：可按供应商、模型和错误类型配置。
- ✅️ 超时控制：Provider 配置支持连接超时、首 token 超时、总请求超时策略，运行时和连接测试按策略计算上游 HTTP 超时并向内部 API 返回当前生效值。
...
- ✅️ MewUI 写入工作流安全接管：供应商、VS Code、分析统计和 VS2026 页面均迁入原生工作台，但仍复用 `/internal` API、安全存储、dry-run、备份、差异预览和二次确认状态。
