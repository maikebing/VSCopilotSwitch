...
- [x] ✅️ 明确当前阶段安全边界：本地代理默认只监听 `127.0.0.1`，配置写入必须 dry-run、备份、幂等且不得记录敏感密钥；许可证和外部贡献流程顺延到公开发布前补齐。
...
- [x] ✅️ 定义当前阶段平台范围：只实现 Windows 桌面端，macOS、Linux、WSL 明确移入后续跨平台阶段。
- [x] ✅️ 明确 Windows 配置路径基线：优先发现 `%APPDATA%\Code\User`，路径解析不得假设固定用户名。
- [x] ✅️ 明确非 Windows 行为：暂不自动写入 macOS、Linux、WSL 配置，后续实现前必须补齐选择策略和测试样例。
- [x] ✅️ 实现本地代理监听基线：管理界面服务使用 `127.0.0.1` 随机可用端口启动，Ollama 调试端口可显式指定。
- [x] ✅️ 默认只监听 `127.0.0.1`，不默认暴露到局域网。
- [x] ✅️ 明确后台代理生命周期基线：当前由 ASP.NET Core 宿主进程管理启动和退出，托盘退出清理在后续宿主层补齐。
- [x] ✅️ 明确托盘能力边界：阶段 1 先落地窗口承载，系统托盘、聚焦窗口和快速切换菜单移入后续 Windows 桌面增强。
...
- [x] ✅️ 实现配置写入向导骨架：选择 VS Code 配置目录、预览差异、确认写入、显示结果。
- [x] ✅️ 实现差异视图：展示 `settings.json` 和 `chatLanguageModels.json` 的托管字段级变化。
- [x] ✅️ 实现回滚入口：列出最近备份并恢复指定备份，恢复前会为当前文件创建安全备份。
- [x] ✅️ 实现高风险动作确认基线：写入配置和恢复备份需要二次确认；包含密钥导出、退出并停止代理将在对应功能实现时补齐确认。
- [x] ✅️ 实现失败修复建议：权限不足、JSON 无效、文件被占用、端口冲突等场景给出可执行处理步骤。
- [x] ✅️ 默认折叠高级选项：代理、熔断阈值、重试次数、备用路由。
...
- [x] ✅️ 实现字段级 diff 输出，返回本项目托管字段的原值、新值和变化状态。
- [x] ✅️ 提供一键回滚到指定备份。
- [x] ✅️ 补齐 VS Code 配置最小测试：写入幂等、备份列表、恢复安全备份。
...
- [ ] 实现主窗口生命周期。
- [x] ✅️ 实现 Windows 托盘图标最小增强：打开或聚焦主界面。
- [x] ✅️ 实现 Windows 托盘菜单最小增强：显示当前提供商和代理服务状态；当前模型后续接入真实路由状态。
- [ ] 实现托盘菜单：快速切换当前提供商。
- [x] ✅️ 实现 Windows 托盘菜单最小增强：退出程序并触发宿主关闭流程，随后停止本地代理。
