...
当前主线：🔴 现在做 阶段 5.5 真实功能闭环。已把“界面保存并启用的供应商”接入 Ollama 代理路由，模型列表从当前启用供应商实时刷新，并新增供应商测试连接；下一步优先补齐 Provider 类型选择，避免所有非 Claude 草稿都按 OpenAI-compatible 路径探测。
...
- ✅️ 新增分析统计页面：展示本地请求日志、监听端口状态、估算 Token、耗时和 User-Agent，且不记录敏感请求头或请求正文。
...
- ✅️ VS Code 暴露模型名追加 `@vscc` 后缀避免与 Copilot 内置模型冲突，代理转发上游前去掉该后缀。
