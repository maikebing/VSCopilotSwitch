...
- ✅️ 确认 OmniHost 窗口 API 的落地方式：Windows 端先采用 `Win32Runtime` + `WebView2AdapterFactory`，托盘 API 后续单独抽象。
...
- 上游错误不会直接泄露敏感字段。
- 当前阶段的基础模型切换链路已接入 UI 启用供应商；测试连接已落地，VS Code 写入当前可用模型、Provider 类型选择和托盘快速切换仍在阶段 5.5 后续任务中补齐。
...
- 🟡 可并行 将 Vue 3 SPA 构建产物作为嵌入式资源打包到单体应用。
